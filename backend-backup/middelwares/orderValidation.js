...
exports.validateOrder = async(req, res, next) => {
    ...
    let minOrderwholesaler = settingData.min_order_wholesaler;
    let minOrder = settingData.min_order;
    let acceptMinOrder = settingData.accept_minimum_order;
    let DeliveryChrge = settingData.delivery_chrge; 
    let DeliveryMaxDay = settingData.delivery_max_day;
    let delivery_day_after_order = settingData.delivery_day_after_order;
    ...
    //-----Order time slot is available or not
    ...
    let order_variants = order.order_val;
    ...
    order_variants.forEach((ele) => {
        ...
        db_products = franchiseProducts.filter((fpEle) => {
            return (fpEle._id == ele.frproductId);
        });
        ...
        if (max_order) {
            let temp = ordered_variants.filter((ov_ele) => {
                return (ov_ele.frproductId == db_products[0]._id);
            });
        ...
        let db_ordered_variants = db_products_variants.filter((vEle) => {
            return (ele.frproductvarId == vEle._id)
        });  
