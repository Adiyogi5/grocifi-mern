...
const Permission = () => {
    ...
    const getRolePermission = useCallback(async () => {
        try {
            setLoading(true);
            const roleData = await AxiosHelper.getData(`role/${id}`);
            if (roleData?.data?.status === true) {
    ...
    const onPermissionChange = async (module, field) => {
        ...
        try {
            ...
            const response = await AxiosHelper.postData(`update_role_permission/${id}`, {
                module,
                permissions: {
