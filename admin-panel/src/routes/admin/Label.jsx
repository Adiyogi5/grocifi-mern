...
import { STATUS , LABELTYPE, LABELPLACE} from '../../constant/fromConfig';
...
const Label = () => {
    ...
    const [initialValues, setInitialValues] = useState({  type:"", label:"", location:"",value:"", color_code:"", status:""})
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("label-datatable", param);
        if (data?.status === true) {
   ...
   const validSchema = Yup.object().shape({
    ...
    const editData = async (event) => {
        var { id, type, label, location,value, color_code, status } = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
        setInitialValues({ id, type, label, location, value, color_code, status });
        setErrors({ name: '', id: '' })
        setFormType('edit')
        setShow(true);
    }
                                    ...
                                    <div className="table-responsive1 ">
                                        <table className="table table-bordered table-striped fs--1 mb-0">
                                            <thead className="bg-200 text-900">
                                                <tr>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "label" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="label">Label</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "location" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="location">Place Location</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "value" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="value">Value</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "color_code" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="color_code">Color code</th>
                                                    <th>Status</th>
                                                    <th>Action</th>
                                                </tr>
                                            </thead>
                                                ...
                                                {data?.record && data?.record.map((row, i) => {
                                                    return (
                                                        <tr key={i}>
                                                            <td>{row.label}</td>
                                                            <td>{row.location}</td>
                                                            <td>{row.value || ""} {row.type || ""}</td>
                                                            <td>{row.color_code || ""}</td>
                                                            <td><Status table='labels' status={row.status} data_id={row._id} /></td>                                                  
            ...
            <Modal size="lg" show={show} fullscreen={false} onHide={() => setShow(false)} centered>
                ...
                <Modal.Body>
                    <MyFrom errors={errors} onSubmit={async (valuesData) => {
                        ...
                        var data = await AxiosHelper.putData(`label/edit/${valuesData.id}`, valuesData);
