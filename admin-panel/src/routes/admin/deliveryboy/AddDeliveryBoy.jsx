...
const AddDeliveryBoy = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await AxiosHelper.getData('delivery-boys/add');
        if (data.status) {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const { data } = await AxiosHelper.postData('delivery-boys/add', formData, true);
      if (data.status) {
          ...
          <div className="card-header">
            <div className="row flex-between-end">
              <div className="col-auto align-self-center">
                <h5 className="mb-0">Add Delivery Boy</h5>
              </div>
