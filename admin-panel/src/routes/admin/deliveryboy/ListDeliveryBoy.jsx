...
const ListDeliveryBoy = () => {
  ...
  const getDataForTable = useCallback(async () => {
    try {
      const { data } = await AxiosHelper.getData('delivery-boys', param);
      if (data?.status === true) {
  ...
  const deleteData = async (event) => {
    ...
    if (isConfirmed) {
      ...
      const { data } = await AxiosHelper.deleteData(`deliveryboy/delete/${id}`);
      if (data?.status === true) {
        getDataForTable();
  ...
  const dropList = [
    ...
    },
    {
      ...
      onClick: (event) => {
        const data = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
        navigate(`/admin/delivery-boys/edit/${data._id}`);
      },
    },
    {
      name: 'Delete',
  ...
  return (
    <div>
              ...
              <div className="row flex-between-end">
                ...
                <div className="col-auto ms-auto">
                  <div className="mt-2" role="tablist">
                    ...
                    <PermissionBlock module="deliveryboy" action="add">
                      <Link to="/admin/delivery-boys/add">
      ...
      >
        <Modal.Header>
          <Modal.Title id="example-modal-sizes-title-lg">View Delivery Boy</Modal.Title>
