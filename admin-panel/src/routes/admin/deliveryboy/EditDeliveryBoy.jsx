...
const EditDeliveryBoy = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await AxiosHelper.getData(`delivery-boys/edit/${id}`);
        if (data.status) {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const { data } = await AxiosHelper.postData(`delivery-boys/edit/${id}`, formData, true);
      if (data.status) {
          ...
          <div className="card-header">
            <div className="row flex-between-end">
              <div className="col-auto align-self-center">
                <h5 className="mb-0">Edit Delivery Boy</h5>
              </div>
              <div className="col-auto ms-auto">
                <Link to="/admin/delivery-boys" className="btn btn-sm btn-falcon-default">
