...
import { AddOnPlan_TYPE, CURRENCY, STATUS } from "../../../constant/fromConfig";
import useRowSelection from "../../../helper/helperFunctions";
const MySwal = withReactContent(Swal);

const AddOnPlan = () => {
  const table = "add_on_plans";
  ...
  const getDataForTable = useCallback(async () => {
    const { data } = await AxiosHelper.getData("addon-plan-datatable", param);
    if (data?.status === true) {
  ...
  const handlePlanTypeChange = useCallback(async (selectedPricingId) => {
    try {
      if (selectedPricingId === "ads") {
        setShowduration(false); // Hide the price field
      } else {
  ...
  const fields = [
    ...
    },
    {
      ...
      name: "type",
      type: "select2",
      options: AddOnPlan_TYPE,
      onChangeCustom: (selected) => {
        handlePlanTypeChange(selected.id);
      },
      col: 6,
    },
    {
  ...
  const deleteData = async (event) => {
    ...
    if (isConfirmed) {
      ...
      var { data } = await AxiosHelper.deleteData(`delete-record/add_on_plans/${id}`);
      if (data?.status === true) {
        getDataForTable();
  ...
  return (
    <>
              ...
              <div className="row flex-between-end">
                <div className="col-auto align-self-center">
                  <h5 className="mb-0" data-anchor="data-anchor">
                    Manage Add-on Plan : : Add-on Plan
                  </h5>
                </div>
                    ...
                    <table className="table table-bordered table-striped fs--1 mb-0">
                      <thead className="bg-200 text-900">
                        <tr>
                          ...
                          <th>Type</th>
                          <th>Price / Duration / Ads Balance</th>
                          <th>Status</th>
                          <th>Action</th>
                        </tr>
                      </thead>
                          ...
                          data?.record?.map((row, i) => {
                            return (
                              <tr key={i}>
                                ...
                                <td>
                                  <div
                                    className="p-2 border rounded shadow-sm bg-light d-flex justify-content-around align-items-center mt-2"
                                    style={{ fontSize: "13px" }}
                                  >
                                    <div className="d-flex flex-column align-items-center">
                                      <span className="fw-bold"> Price:</span>
                                      <span className="badge bg-primary fs13">
                                        {" "}
                                        {CURRENCY + " " + row?.price}
                                      </span>
                                    </div>
                                    {row?.type === "life" && (
                                      <div className="d-flex flex-column align-items-center">
                                        ...
                                        <span className="badge bg-warning fs13">
                                          {row?.duration} Days{" "}
                                        </span>
                                      </div>
                                    )}
                                    {row?.type === "ads" && (
                                      <div className="d-flex flex-column align-items-center">
                                        ...
                                        <span className="badge bg-dark fs13">
                                          {row?.ads_balance} Ads
                                        </span>
                                      </div>
                                    )}
      ...
      >
        <Modal.Header>
          <Modal.Title>{ucFirst(formType)} Add-on Plan</Modal.Title>
          <CloseButton
        ...
        <Modal.Body>
          <MyFrom
            ...
            onSubmit={async (valuesData) => {
              ...
              var data = "";
              if (formType === "add") {
                data = await AxiosHelper.postData("addon-plan", valuesData);
              } else {
                data = await AxiosHelper.putData(
                  `addon-plan/edit/${valuesData.id}`,
                  valuesData
                );
              }
