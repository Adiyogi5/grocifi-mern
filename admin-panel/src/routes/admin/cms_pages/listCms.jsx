...
const listCms = () => {
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("cms_page", param);
        if (data?.status === true) {
    ...
    const deleteData = async (event) => {
        ...
        if (isConfirmed) {
            ...
            var { data } = await AxiosHelper.deleteData(`delete-record/cmspages/${id}`);
            if (data?.status === true) {
                getDataForTable()
    ...
    const dropList = [
        ...
        },
        {
            ...
            onClick: (event) => {
                var data = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
                navigate(`/admin/cms_page/edit/${data.slug}`)
            }
        },
        {
            name: "Delete",
                        ...
                        <div className="card-header">
                            <div className="row flex-between-end">
                                <div className="col-auto align-self-center">
                                    <h5 className="mb-0" data-anchor="data-anchor">Cms Page</h5>
                                </div>
                                        ...
                                        <table className="table table-bordered table-striped fs--1 mb-0">
                                            <thead className="bg-200 text-900">
                                                <tr>
                                                    ...
                                                    <th>Image</th>
                                                    <th>Page</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "title" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="name">Title</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "status" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="status">Status</th>
            ...
            >
                ...
                <ul className="list-group">
                    ...
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        ...
                        <div dangerouslySetInnerHTML={{ __html: initialValues?.content }}/></span>
                    </li>
             
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Meta Title</label>
                        <span className="fs--1 text-muted ps-3">{initialValues?.seo?.metaTitle}</span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Meta Keyword</label>
                        <span className="fs--1 text-muted ps-3">{initialValues?.seo?.metaKeywords}</span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Meta Description</label>
                        <span className="fs--1 text-muted ps-3">{initialValues?.seo?.metaDescription}</span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Sections</label>
                        <span className="fs--1 text-muted ps-3">{initialValues?.sections}</span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Status</label>
                        <Status table='cmspages' status={initialValues.status} data_id={initialValues._id} />
                    </li>
                </ul>
                </Modal.Body>
            </Modal>
