...
const FaqRequest = () => {
  ...
  const getDataForTable = useCallback(async () => {
    const { data } = await AxiosHelper.getData("faqrequest-datatable", param);
    if (data?.status === true) {
  ...
  const deleteData = async (event) => {
    ...
    if (isConfirmed) {
      ...
      var { data } = await AxiosHelper.deleteData(`delete-record/faqrequests/${id}`);
      if (data?.status === true) {
        getDataForTable();
              ...
              <div className="row flex-between-end">
                <div className="col-auto align-self-center">
                  <h5 className="mb-0" data-anchor="data-anchor">
                    Manage Faq Requests :: Faq Requests List
                  </h5>
                </div>
                      ...
                      <thead className="bg-200 text-900">
                        <tr>
                          ...
                          <th
                            onClick={handelSort}
                            className={`sort ${
                              param?.orderBy === "register_date" &&
                              (param?.orderDirection === 1 ? "asc" : "desc")
                            }`}
                            data-sort="register_date"
                          >
                            Register Date
                          </th>
                          <th>Action</th>
