...
const listBlogAuthor = () => {
    const table = "blogauthors"
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("blogauthor-datatable", param);
    ...
    const deleteData = async (event) => {
        ...
        if (isConfirmed) {
            ...
            var { data } = await AxiosHelper.deleteData(`delete-record/blogauthors/${id}`);
            if (data?.status === true) {
                getDataForTable()
    ...
    const dropList = [
        ...
        },
        {
            ...
            onClick: (event) => {
                var data = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
                navigate(`/admin/blogauthor/edit/${data.slug}`)
            }
        },
        {
            name: "Delete",
                        ...
                        <div className="card-header">
                            <div className="row flex-between-end">
                                <div className="col-auto align-self-center">
                                    <h5 className="mb-0" data-anchor="data-anchor">Blog Author</h5>
                                </div>
                                <div className="col-auto ms-auto">
                                    <div className="mt-2" role="tablist">
                                        ...
                                        <PermissionBlock module={'BlogAuthor'} action={'add'}>
                                            <Link to={`/admin/blogauthor/add`}>
                                        ...
                                        <table className="table table-bordered table-striped fs--1 mb-0">
                                            <thead className="bg-200 text-900">
                                                <tr>
                                                    ...
                                                    <th>Image</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "author_name" && (param?.author_name === 1 ? 'asc' : 'desc')}`} data-sort="author_name">Auther Name</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "about_author" && (param?.about_author === 1 ? 'asc' : 'desc')}`} data-sort="about_author">About Auther</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "designation_id.name" && (param?.designation_id.name === 1 ? 'asc' : 'desc')}`} data-sort="designation_id.name">Designation</th>
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "business_id.business_name" && (param?.business_id.business_name === 1 ? 'asc' : 'desc')}`} data-sort="business_id.name">Business</th>
                                                ...
                                                {data?.record && data?.record.map((row, i) => {
                                                    return (
                                                        <tr key={i}>
                                                            ...
                                                            <td className="fw-bold text-primary cursor-pointer" main-data={JSON.stringify(row)} onClick={viewData}>{row?.author_name}</td>
                                                            <td>{row.about_author}</td>
                                                            <td>{row.designation_id?.name} </td>
                                                            <td>{row.business_id?.business_name} </td>
                                                            <td>{formatDateDDMMYYYY(row.createdAt)}</td>
                                                            <td><Status table='blogauthors' status={row.status} data_id={row._id} /></td>
            ...
            >
                ...
                <ul className="list-group">
                    ...
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        ...
                        <span className="fs--1 text-muted">{initialValues?.author_name} </span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">About Author</label>
                        <span className="fs--1 text-muted">{initialValues?.about_author} </span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Designation</label>
                        <span className="fs--1 text-muted">{initialValues?.designation_id?.name} </span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Business</label>
                        <span className="fs--1 text-muted">{initialValues.business_id?.name} </span>
                    </li>
                
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Youtube URL</label>
                        <span className="fs--1">{initialValues?.youtube_url}</span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Facebook URL</label>
                        <span className="fs--1">{initialValues?.facebook_url}</span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Linkedin URL</label>
                        <span className="fs--1">{initialValues?.linkedin_url}</span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">Instagram URL</label>
                        <span className="fs--1">{initialValues?.instagram_url}</span>
                    </li>
                    <li className="list-group-item d-flex justify-content-between align-items-center">
                        <label className="fs--1 mb-0">X URL</label>
                        <span className="fs--1">{initialValues?.x_url}</span>
                    </li>
