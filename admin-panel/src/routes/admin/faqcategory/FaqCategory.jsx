...
const FaqCategory = () => {
    ...
    const [initialValues, setInitialValues] = useState({ name: '',description:"", status: 1 })
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("faqCategory-datatable", param);
        if (data?.status === true) {
    ...
    const editData = async (event) => {
        var { id, name,description, status } = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
        setInitialValues({ id, name, description, status });
        setErrors({ name: '', id: '' })
        setFormType('edit')
        setShow(true);
    }
    ...
    const deleteData = async (event) => {
        ...
        if (isConfirmed) {
            ...
            var { data } = await AxiosHelper.deleteData(`delete-record/faqcategories/${id}`);
            if (data?.status === true) {
                getDataForTable()
    ...
    return (
        <>
                            ...
                            <div className="row flex-between-end">
                                <div className="col-auto align-self-center">
                                    <h5 className="mb-0" data-anchor="data-anchor">
                                        Manage Faq Category :: Faq Category List
                                    </h5>
                                </div>
                                <div className="col-auto ms-auto">
                                    ...
                                    <div className="mt-2" role="tablist">
                                        ...
                                        <PermissionBlock module={'faqCategory'}>
                                            <button onClick={() => {
                                                ...
                                                {data?.record && data?.record.map((row, i) => {
                                                    return (
                                                        <tr key={i}>
                                                            ...
                                                            <td className="fw-bold" >{row.description}</td>
                                                            <td><Status table='designations' status={row.status} data_id={row._id} /></td>
            ...
            <Modal size="md" show={show} fullscreen={false} onHide={() => setShow(false)} centered>
                <Modal.Header>
                    <Modal.Title>{ucFirst(formType)} Faq Category</Modal.Title>
                    <CloseButton
                ...
                <Modal.Body>
                    <MyFrom errors={errors} onSubmit={async (valuesData) => {
                        ...
                        var data = "";
                        if (formType === 'add') {
                            data = await AxiosHelper.postData("faqCategory/add", valuesData);
                        } else {
                            data = await AxiosHelper.putData(`faqCategory/edit/${valuesData.id}`, valuesData);
                        }
                ...
                <Modal.Body>
                    <ul className="list-group">
                        <li className="list-group-item d-flex justify-content-between align-items-center">
                            <label className='fs--1 m-0'>Category Name</label>
                            <span className="fs--1">{initialValues?.name}</span>
                        </li>
                        <li className="list-group-item d-flex justify-content-between align-items-center">
                            <label className='fs--1 m-0'>Description</label>
