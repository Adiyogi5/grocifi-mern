...
const Sponsor = () => {
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("sponsor-datatable", param);
        if (data?.status === true) {
    ...
    const editData = async (event) => {
        var { id, name, image, status } = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
        setInitialValues({ id, name, image, status });
        setErrors({ name: '', id: '' })
        setFormType('edit')
        setShow(true);
    }
    ...
    const deleteData = async (event) => {
        ...
        if (isConfirmed) {
            ...
            var { data } = await AxiosHelper.deleteData(`delete-record/sponsors/${id}`);
            if (data?.status === true) {
                getDataForTable()
    ...
    return (
        <>
                            ...
                            <div className="row flex-between-end">
                                ...
                                <div className="col-auto ms-auto">
                                    ...
                                    <div className="mt-2" role="tablist">
                                        ...
                                        <PermissionBlock module={'Sponsor'}>
                                            <button onClick={() => {
                                                setInitialValues({ name:"", image:"", status:"" })
                                                setErrors({ name: '', id: '' })
                                                setFormType('add')
                                                setShow(true)
                                                ...
                                                {data?.record && data?.record.map((row, i) => {
                                                    return (
                                                        <tr key={i}>
                                                            ...
                                                            <td className="fw-bold text-primary cursor-pointer" main-data={JSON.stringify(row)} >{row.name}</td>
                                                            <td><Status table='sponsors' status={row.status} data_id={row._id} /></td>
            ...
            <Modal size="md" show={show} fullscreen={false} onHide={() => setShow(false)} centered>
                <Modal.Header>
                    <Modal.Title>{ucFirst(formType)} Sponsor</Modal.Title>
                    <CloseButton
                ...
                <Modal.Body>
                    <MyFrom errors={errors} onSubmit={async (valuesData) => {
                        ...
                        var data = "";
                        if (formType === 'add') {
                            data = await AxiosHelper.postData("sponsor/add", valuesData,true) ;
                        } else {
                            data = await AxiosHelper.putData(`sponsor/edit/${valuesData.id}`, valuesData, true);
                        }
