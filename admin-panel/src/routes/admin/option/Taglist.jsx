...
const Taglist = () => {
  ...
  const [errors, setErrors] = useState({ name: "", type: "", status: "" });

  const getDataForTable = useCallback(async () => {
    const { data } = await AxiosHelper.getData("tag-datatable", param);
    if (data?.status === true) {
  ...
  const validSchema = Yup.object().shape({
    ...
    type: Yup.string()
      .oneOf(["business", "blog", "ad"])
      .required("Select one of the tag type available the options."),
  ...
  const editData = async (event) => {
    var { id, name, type, status } = JSON.parse(
      event.target.attributes.getNamedItem("main-data").value
    );
    setInitialValues({ id, name, type, status });
    setErrors({ name: "", type: "", id: "" });
    setFormType("edit");
    setShow(true);
  };
  ...
  const deleteData = async (event) => {
    ...
    if (isConfirmed) {
      ...
      var { data } = await AxiosHelper.deleteData(`delete-record/tags/${id}`);
      if (data?.status === true) {
        getDataForTable();
  ...
  return (
    <>
              ...
              <div className="row flex-between-end">
                ...
                <div className="col-auto ms-auto">
                  <div className="mt-2" role="tablist">
                    ...
                    <PermissionBlock module={"Tag"} action={"add"}>
                      <button
                        onClick={() => {
                          setInitialValues({ name: "", type: "", status: 1 });
      ...
      >
        <Modal.Header>
          <Modal.Title>{ucFirst(formType)} Tag</Modal.Title>
          <CloseButton
        ...
        <Modal.Body>
          <MyFrom
            ...
            onSubmit={async (valuesData) => {
              ...
              var data = "";
              if (formType === "add") {
                data = await AxiosHelper.postData("tagcreate", valuesData);
              } else {
                data = await AxiosHelper.putData(
                  `tagedit/${valuesData.id}`,
                  valuesData
                );
              }
