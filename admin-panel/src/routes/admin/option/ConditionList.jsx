...
const ConditionList = () => {
  ...
  const getDataForTable = useCallback(async () => {
    const { data } = await AxiosHelper.getData("condition-datatable", param);
    if (data?.status === true) {
  ...
  const editData = async (event) => {
    var { id, name, status, condition_type } = JSON.parse(
      event.target.attributes.getNamedItem("main-data").value
    );
    setInitialValues({ id, name, condition_type, status });
    setErrors({ name: "", id: "" });
    setFormType("edit");
    setShow(true);
  };
  ...
  return (
    <>
              ...
              <div className="row flex-between-end">
                <div className="col-auto align-self-center">
                  <h5 className="mb-0" data-anchor="data-anchor">
                    Manage Condition :: Condition List
                  </h5>
                </div>
      ...
      >
        <Modal.Header>
          <Modal.Title>{ucFirst(formType)} Condition </Modal.Title>
          <CloseButton
