...
const CurrencyList = () => {
  ...
  const getDataForTable = useCallback(async () => {
    const { data } = await AxiosHelper.getData("currency-datatable", param);
    if (data?.status === true) {
  ...
  return (
    <>
              ...
              <div className="row flex-between-end">
                ...
                <div className="col-auto ms-auto">
                  <div className="mt-2" role="tablist">
                    ...
                    <PermissionBlock module={"Currency"} action={"add"}>
                      <button
                        onClick={() => {
      ...
      >
        <Modal.Header>
          <Modal.Title>{ucFirst(formType)} Currency</Modal.Title>
          <CloseButton
        ...
        <Modal.Body>
          <MyFrom
            ...
            onSubmit={async (valuesData) => {
              ...
              var data = "";
              if (formType === "add") {
                data = await AxiosHelper.postData("currencycreate", valuesData);
              } else {
                data = await AxiosHelper.putData(
                  `currencyedit/${valuesData.id}`,
                  valuesData
                );
              }
