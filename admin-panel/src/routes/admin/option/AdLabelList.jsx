...
const AdLabelList = () => {
  ...
  const getDataForTable = useCallback(async () => {
    const { data } = await AxiosHelper.getData("adlabel-datatable", param);
    if (data?.status === true) {
  ...
  const editData = async (event) => {
    var { id, name, label_type, status } = JSON.parse(
      event.target.attributes.getNamedItem("main-data").value
    );
    setInitialValues({ id, name, label_type, status });
    setErrors({ name: "", id: "" });
    setFormType("edit");
    setShow(true);
  };
  ...
  return (
    <>
              ...
              <div className="row flex-between-end">
                <div className="col-auto align-self-center">
                  <h5 className="mb-0" data-anchor="data-anchor">
                    Manage Ads Label :: Ads Label List
                  </h5>
                </div>
                <div className="col-auto ms-auto">
                  <div className="mt-2" role="tablist">
                    ...
                    <PermissionBlock module={"AdLabel"} action={"add"}>
                      <button
                        onClick={() => {
      ...
      >
        <Modal.Header>
          <Modal.Title>{ucFirst(formType)} Ads Label </Modal.Title>
          <CloseButton
        ...
        <Modal.Body>
          <MyFrom
            ...
            onSubmit={async (valuesData) => {
              ...
              var data = "";
              if (formType === "add") {
                data = await AxiosHelper.postData("adlabelcreate", valuesData);
              } else {
                data = await AxiosHelper.putData(
                  `adlabeledit/${valuesData.id}`,
                  valuesData
                );
              }
