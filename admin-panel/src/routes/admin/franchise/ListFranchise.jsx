...
const ListFranchise = () => {
  ...
  const getDataForTable = useCallback(async () => {
    try {
      const { data } = await AxiosHelper.getData('franchise', param);
      if (data?.status === true) {
  ...
  const deleteData = async (event) => {
    ...
    if (isConfirmed) {
      ...
      const { data } = await AxiosHelper.deleteData(`franchise/delete/${id}`);
      if (data?.status === true) {
        getDataForTable();
  ...
  const dropList = [
    ...
    },
    {
      ...
      onClick: (event) => {
        const data = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
        navigate(`/admin/franchise/edit/${data._id}`);
      },
    },
    {
      name: 'Delete',
  ...
  return (
    <div>
              ...
              <div className="row flex-between-end">
                ...
                <div className="col-auto ms-auto">
                  <div className="mt-2" role="tablist">
                    ...
                    <PermissionBlock module="franchise" action="add">
                      <Link to="/admin/franchise/add">
                    ...
                    <table className="table table-bordered table-striped fs--1 mb-0">
                      <thead className="bg-200 text-900">
                        <tr>
                          ...
                          <th
                            onClick={handleSort}
                            className={`sort ${param?.orderBy === 'firmname' && (param?.orderDirection === 1 ? 'asc' : 'desc')}`}
                            data-sort="firmname"
                          >
                            Firm Name
                          </th>
                          <th
                            onClick={handleSort}
                            className={`sort ${param?.orderBy === 'ownermobile' && (param?.orderDirection === 1 ? 'asc' : 'desc')}`}
                            data-sort="ownermobile"
                          >
                            Owner Mobile
                          </th>
                          <th
                      ...
                      <tbody className="list">
                        {data?.record && data?.record.map((row, i) => (
                          <tr key={i}>
                            ...
                            <td>
                              <img src={row.logo || row.userId?.img} alt="" width={100} />
                            </td>
                            <td
                            ...
                            <td>{row.userId?.phone_no}</td>
                            <td>{formatDateDDMMYYYY(row.created)}</td>
                            <td>
                              <Status table="franchises" status={row.status} data_id={row._id} />
                            </td>
                            <td>
      ...
      >
        <Modal.Header>
          <Modal.Title id="example-modal-sizes-title-lg">View Franchise</Modal.Title>
          <CloseButton onClick={() => setShowView(false)} />
        </Modal.Header>
        <Modal.Body>
          <ul className="list-group">
            <li className="list-group-item text-center">
              <img src={initialValues.logo || initialValues.userId?.img} alt="Thumbnail" className="img-fluid" width={250} />
            </li>
            <li className="list-group-item d-flex justify-content-between align-items-center">
              <label className="fs--1 mb-0">Name</label>
              <span className="fs--1 text-muted">{initialValues.userId?.fname + ' ' + initialValues.userId?.lname}</span>
            </li>
            <li className="list-group-item d-flex justify-content-between align-items-center">
              <label className="fs--1 mb-0">Firm Name</label>
              <span className="fs--1">{initialValues.firmname}</span>
            </li>
            <li className="list-group-item d-flex justify-content-between align-items-center">
              <label className="fs--1 mb-0">Owner Mobile</label>
              <span className="fs--1">{initialValues.ownermobile}</span>
            </li>
            <li className="list-group-item d-flex justify-content-between align-items-center">
              <label className="fs--1 mb-0">Contact Person</label>
              <span className="fs--1">{initialValues.contactpersonname}</span>
            </li>
            <li className="list-group-item d-flex justify-content-between align-items-center">
              <label className="fs--1 mb-0">Email</label>
              <span className="fs--1">{initialValues.userId?.email}</span>
            </li>
            <li className="list-group-item d-flex justify-content-between align-items-center">
              <label className="fs--1 mb-0">Mobile</label>
              <span className="fs--1 text-muted ps-3">{initialValues.userId?.phone_no}</span>
            </li>
            <li className="list-group-item d-flex justify-content-between align-items-center">
              <label className="fs--1 mb-0">Status</label>
              <Status table="franchises" status={initialValues.status} data_id={initialValues._id} />
            </li>
            <li className="list-group-item d-flex justify-content-between align-items-center">
