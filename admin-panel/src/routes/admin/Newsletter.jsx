...
const Newsletter = () => {
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("newsletter-datatable", param);
        if (data?.status === true) {
    ...
    const deleteData = async (event) => {
        ...
        if (isConfirmed) {
            ...
            var { data } = await AxiosHelper.deleteData(`delete-record/newsletters/${id}`);
            if (data?.status === true) {
                getDataForTable()
                            ...
                            <div className="row flex-between-end">
                                <div className="col-auto align-self-center">
                                    <h5 className="mb-0" data-anchor="data-anchor">
                                        Manage Newsletter :: Newsletter List
                                    </h5>
                                </div>
                                        ...
                                        <table className="table table-bordered table-striped fs--1 mb-0">
                                            <thead className="bg-200 text-900">
                                                <tr>
                                                    ...
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "register_date" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="register_date">Register Date</th>
                                                    <th>Action</th>
                                                </tr>
                                            </thead>
                                                ...
                                                {data?.record && data?.record.map((row, i) => {
                                                    return (
                                                        <tr key={i}>
                                                            ...
                                                            <td className="fw-bold text-primary cursor-pointer" main-data={JSON.stringify(row)} >{row.email}</td>
