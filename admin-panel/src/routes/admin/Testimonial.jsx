...
import { STATUS, RATING} from '../../constant/fromConfig';
...
const Testimonial = () => {
    ...
    const [initialValues, setInitialValues] = useState({ name: '',description:"",designation:"",title:"",rating:"",status: 1 })
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("testimonial-datatable", param);
        if (data?.status === true) {
    ...
    const editData = async (event) => {
        var { id, name, image,description,designation,title,rating, status } = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
        setInitialValues({ id, name, image, description, designation, title, rating, status });
        setErrors({ name: '', id: '' })
        setFormType('edit')
        setShow(true);
    }
    ...
    const deleteData = async (event) => {
        ...
        if (isConfirmed) {
            ...
            var { data } = await AxiosHelper.deleteData(`delete-record/testimonials/${id}`);
            if (data?.status === true) {
                getDataForTable()
    ...
    return (
        <>
                            ...
                            <div className="row flex-between-end">
                                <div className="col-auto align-self-center">
                                    <h5 className="mb-0" data-anchor="data-anchor">
                                        Manage Testimonial:: Testimonial List
                                    </h5>
                                </div>
                                <div className="col-auto ms-auto">
                                    ...
                                    <div className="mt-2" role="tablist">
                                        ...
                                        <PermissionBlock module={'Testimonial'}>
                                            <button onClick={() => {
                                                setInitialValues({ name:"", image:"",description:"",title:"",rating:"",designation:"",status:"" })
                                                setErrors({ name: '', id: '' })
                                                setFormType('add')
                                                setShow(true)
                                        ...
                                        <table className="table table-bordered table-striped fs--1 mb-0">
                                            <thead className="bg-200 text-900">
                                                <tr>
                                                    ...
                                                    <th onClick={handelSort} className={`sort ${param?.orderBy === "description" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="description">Description</th>
                                                    <th>Status</th>
                                                    <th>Action</th>
                                                </tr>
                                            </thead>
                                                ...
                                                {data?.record && data?.record.map((row, i) => {
                                                    return (
                                                        <tr key={i}>
                                                            ...
                                                            <td>{row.title}<Rating rating={row.rating} /></td>
            ...
            <Modal size="lg" show={show} fullscreen={false} onHide={() => setShow(false)} centered>
                <Modal.Header>
                    <Modal.Title>{ucFirst(formType)} Testimonial</Modal.Title>
                    <CloseButton
                ...
                <Modal.Body>
                    <MyFrom errors={errors} onSubmit={async (valuesData) => {
                        ...
                        var data = "";
                        if (formType === 'add') {
                            data = await AxiosHelper.postData("testimonial/add", valuesData,true) ;
                        } else {
                            data = await AxiosHelper.putData(`testimonial/edit/${valuesData.id}`, valuesData, true);
                        }
