...
const ListWholesaler = () => {
    ...
    const getDataForTable = useCallback(async () => {
        try {
            const { data } = await AxiosHelper.getData('wholesaler', param);
            if (data?.status === true) {
    ...
    const deleteData = async (event) => {
        ...
        if (isConfirmed) {
            ...
            const { data } = await AxiosHelper.deleteData(`wholesaler/delete/${_id}`);
            if (data?.status === true) {
                getDataForTable();
    ...
    const dropList = [
        ...
        },
        {
            ...
            onClick: (event) => {
                const data = JSON.parse(event.target.attributes.getNamedItem('main-data').value);
                navigate(`/admin/wholesaler/edit/${data._id}`);
            }
        },
        {
            name: 'Delete',
    ...
    return (
        <div>
                        ...
                        <div className="card-header">
                            <div className="row flex-between-end">
                                <div className="col-auto align-self-center">
                                    <h5 className="mb-0" data-anchor="data-anchor">Wholesaler</h5>
                                </div>
                                <div className="col-auto ms-auto">
                                    <div className="mt-2" role="tablist">
                                        ...
                                        <PermissionBlock module="Wholesaler" action="add">
                                            <Link to="/admin/wholesaler/add">
                                            ...
                                            <tbody className="list">
                                                {data?.record && data?.record.map((row, i) => (
                                                    <tr key={i}>
                                                        ...
                                                        <td>{row.wholesaler_firmname}</td>
                                                        <td>{row.gst_no}</td>
                                                        <td>{row.email}</td>
                                                        <td>{row.phone_no}</td>
                                                        <td>{formatDateDDMMYYYY(row.created)}</td>
                                                        <td>
                                                            <Status table="wholesaler" status={row.status} data_id={row._id} />
                                                        </td>
                                                        <td>
            ...
            >
                <Modal.Header>
                    <Modal.Title id="example-modal-sizes-title-lg">View Wholesaler</Modal.Title>
                    <CloseButton onClick={() => setShowView(false)} />
                </Modal.Header>
                <Modal.Body>
                    <ul className="list-group">
                        <li className="list-group-item text-center">
                            <img src={initialValues.img} alt="Profile" className="img-fluid" width={250} />
                        </li>
                        {initialValues.visiting_card && (
                            <li className="list-group-item text-center">
                                <img src={initialValues.visiting_card} alt="Visiting Card" className="img-fluid" width={250} />
                            </li>
                        )}
                        ...
                        <li className="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <span className="fs--1 text-muted">{initialValues.phone_no}</span>
                        </li>
                        <li className="list-group-item d-flex justify-content-between align-items-center">
                        ...
                        <li className="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <Status table="wholesaler" status={initialValues.status} data_id={initialValues._id} />
                        </li>
                        <li className="list-group-item d-flex justify-content-between align-items-center">
