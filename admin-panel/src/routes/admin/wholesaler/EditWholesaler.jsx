...
const EditWholesaler = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
           const { data } = await AxiosHelper.getData(`wholesaler/edit/${id}`);
                if (data.status) {
                    setRoles(data.data.roles ? data.data.roles.map((role) => ({ id: role._id, name: role.name })) : [{ id: data.data.roleId._id, name: data.data.roleId.name }]);
                    setInitialValues({
                        fname: data.data.user.fname,
                        lname: data.data.user.lname,
                        email: data.data.user.email,
                        phone_no: data.data.user.phone_no,
                        wholesaler_firmname: data.data.user.wholesaler_firmname,
                        gst_no: data.data.user.gst_no,
                        img: data.data.user.img || '',
                        visiting_card: data.data.user.visiting_card || '',
                        roleId: data.data.user.roleId._id
                    });
                } else {
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            const { data } = await AxiosHelper.postData(`wholesaler/edit/${id}`, formData, true);
            if (data.status) {
    ...
    const fields = [
        ...
        { name: 'roleId', label: 'Role', type: 'select', options: roles, col: 12 },
                    ...
                    <div className="card-header">
                        <div className="row flex-between-end">
                            <div className="col-auto align-self-center">
                                <h5 className="mb-0">Edit Wholesaler</h5>
                            </div>
