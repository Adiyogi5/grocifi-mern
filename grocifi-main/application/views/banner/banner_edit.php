      ...
      <div class="card card-default color-palette-bo">
        ...
        <div class="card-body">   
           <!-- For Messages -->
            ...
            <?php echo form_open_multipart(base_url($model.'/banner/edit/'.$banner['_id']), 'class="form-horizontal" id="bannerForm"');  ?> 
              <div class="form-group row">                
                <div class="col-sm-6">
                  <label for="title" class="col-sm-6 control-label">Banner Name <span class="red">*</span></label>
                  <input type="text" name="title" class="form-control" id="title" placeholder="" value="<?= set_value('title',$banner['title']); ?>">
                </div>
                 <div class="col-sm-6">
                  <label for="holiday_date" class="col-sm-6 control-label">Franchise Name<span class="red">*</span></label>
                   <select name="franchise_id" class="form-control" id="franchise_id" >
                    ...
                    <?php foreach ($franchise as $key => $value) { ?>
                      <option  <?php if(set_value('franchise_id',$banner['franchise_id'])==$value['_id']){ echo "selected='selected'"; }  ?> value="<?=$value['_id']?>"><?=$value['firmname']?></option>
                    <?php } ?>
                  </select>
                  </div>
                </div>
                ...
                <div class="col-6">
                  ...
                  <input type="hidden" name="_img" value="<?=$banner['img']; ?>">
                  <small>Banner size should be 1920X605 px</small>
                  </div>
                  <div class="col-6">
                  <p><img src="<?= $this->config->item('APIIMAGES') ?>banner_img/<?=$banner['img']; ?>" class="logosmallimg"></p> 
                  </div>
                  </div>
                </div>
                <div class="col-6">
                  ...
                  <select name="is_active" id="is_active" class="form-control">
                    ...
                    <option value="1" <?= (set_value('is_active',$banner['is_active']) == '1')?'selected': '' ?>>Active</option>
                    <option value="2" <?= (set_value('is_active',$banner['is_active']) == '2')?'selected': '' ?>>Deactive</option>
                  </select>
                </div>  
              </div>   
              ...
              <div class="form-group">
                <div class="col-md-12">
                  ...
                  <input type="submit" name="submit" value="Update Banner" class="btn btn-info pull-right btn-submit">
                </div>
              </div>
            <?php echo form_close( ); ?>
          <!-- /.box-body -->
