  ...
  <section class="content">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <?php if($this->session->userdata('role_type')!='2' || (isset($this->general_user_premissions['banner']['is_add']) && $this->general_user_premissions['banner']['is_add']==1)){ ?>  
          <a href="<?= base_url($model.'/banner/add'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Banner</a>
        <?php } ?>  
        </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url($model.'/banner/banner_datatable_json')?>?franchise_id="+$('#search_franchise_id').val(),
    "order": [[1,'asc']],
    "columnDefs": [
  ...
  $("body").on("change",".tgl_checkbox",function(){
    $.post('<?=base_url($model."/banner/change_banner_status")?>',
    {
