  ...
  <section class="content">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <?php if($this->session->userdata('role_type')!='2' || (isset($this->general_user_premissions['product']['is_add']) && $this->general_user_premissions['product']['is_add']==1)){ ?>
        <?php if($this->general_settings['is_marg']==1){ ?>
        <div class="d-inline-block float-right" style="margin-left:20px;">  
          <a href="<?= base_url($model.'/product/syncproduct/1'); ?>" class="btn btn-info"><i class="fa 
fa-recycle"></i> Sync New Product (Marg Api)</a> &nbsp;
          <a href="<?= base_url($model.'/product/syncproduct/2'); ?>" class="btn btn-warning"><i class="fa 
fa-recycle"></i> Update Sync Product (Marg Api)</a>
        </div>
      <?php }else{ ?>
        <div class="d-inline-block float-right"> 
          <a href="<?= base_url($model.'/product/addproduct'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Product</a>
        </div>
      <?php } ?>
      <?php } ?>
      </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url($model.'/product/product_datatable_json')?>",
    "order": [[1,'asc']],
    "columnDefs": [
    { "targets": 0, "name": "_id", 'searchable':false, 'orderable':false}, 
    { "targets": 1, "name": "title", 'searchable':true, 'orderable':true},
    { "targets": 2, "name": "catName", 'searchable':true, 'orderable':true},
    { "targets": 3, "name": "created", 'searchable':false, 'orderable':true}, 
    { "targets": 4, "name": "is_global", 'searchable':false, 'orderable':true},
  ...
  $("body").on("change",".tgl_checkbox",function(){
    $.post('<?=base_url($model."/product/change_product_status")?>',
    {
