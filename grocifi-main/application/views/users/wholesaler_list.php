    ...
    <div class="card">
      <div class="card-header" style="margin-bottom: 5px;">
        <div class="d-inline-block">
          <h3 class="card-title"><i class="fa fa-list"></i>&nbsp; Wholesalers List </h3>
        </div>
        <div class="d-inline-block float-right"> 
          <?php if($this->session->userdata('role_type')!='2' || (isset($this->general_user_premissions['user_wholesaler']['is_add']) && $this->general_user_premissions['user_wholesaler']['is_add']==1)){ ?> 
          <a href="<?= base_url($model.'/users/add/wholesaler'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Wholesaler</a>
          <?php } ?>
        </div>
      </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url($model.'/users/wholesaler_datatable_json')?>",
    "order": [[7,'desc']],
    "columnDefs": [
    ...
    { "targets": 8, "name": "is_active", 'searchable':false, 'orderable':false},
    { "targets": 9, "name": "is_wholesaler_approve", 'searchable':false, 'orderable':false},
  ...
  $("body").on("click",".updateWallet",function(e){
      e.preventDefault();
      var loading = '<?=base_url($model."/assets/img/loading.gif")?>';
