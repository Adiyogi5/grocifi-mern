  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="card card-default color-palette-bo">
        ...
        <div class="card-body">  
          <?php          
           echo form_open(base_url($model.'/users/franchiseproducts/'.$franchise['_id']), 'class="form-horizontal" id="fraproductForm"');  ?>          
           <!-- For Messages -->
            <?php $this->load->view('includes/_messages.php') ?>  
                ...
                <div class="col-sm-6">
                  ...
                  <select name="categoryId" class="form-control select2" id="categoryId" >
                    ...
                    <?php foreach ($catagory as $key => $value) { ?>
                      <option <?php if(set_value('categoryId')==$value['_id']){ echo "selected='selected'"; }  ?> value="<?=$value['_id']?>"><?=$value['title']?></option>
                    <?php } ?>
                  </select>
              ...
              <div class="form-group row">                
                <div class="col-sm-6">
                  <label for="isPacket" class="col-sm-12 control-label">Package Type <span class="red">*</span></label>
                    <label style="font-weight: normal;"><input value="1" type="radio" checked="checked" name="isPacket">&nbsp;Packet</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <label style="font-weight: normal;"><input value="0" type="radio" name="isPacket">&nbsp;Loose</label>
                </div> 
                 <div class="col-md-6">
              ...
              <div class="form-group row">                
                <div class="col-sm-6">
                  <label for="categoryId" class="col-sm-12 control-label">Max Order <span class="red">*</span><small>(Zero value means unlimited)</small></label>
                   <input type="text" name="product_max_order" id="product_max_order" class="form-control" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g, '')"  style="width:40%;float: left; margin-right:20px;" value="<?= (set_value('product_max_order'));?>">  
                   <select name="product_unit"  id="product_unit" class="form-control"  style="width:40%;float: left;">
                    <?php foreach ($this->config->item('units') as $ukey => $uvalue) { ?>
                      <option value="<?=$ukey?>" <?= (set_value('product_unit') == $ukey)?'selected': '' ?>><?=$uvalue?></option>
                    <?php  }?>
                  </select>
                  <label id="product_max_order-error" class="error" for="product_max_order"></label>
                </div> 
                 <div class="col-md-6">
                  <label for="is_active" class="col-sm-6 control-label">Product Quality <span class="red">*</span></label>                  
                   <select name="product_quality"  id="product_quality" class="form-control">
                    <?php foreach ($this->config->item('proquality') as $ukey => $uvalue) { ?>
                      <option value="<?=$ukey?>" <?= (set_value('product_quality') == $ukey)?'selected': '' ?>><?=$uvalue?></option>
                    <?php  }?>
                  </select>
                </div> 
              </div> 
                ...
                <div class="col-md-6">
                  <label for="is_active" class="col-sm-6 control-label">Show On Front<span class="red">*</span></label>                  
                   <select name="isShown"  id="isShown" class="form-control"> 
                      <option value="true" <?= (set_value('isShown') == 1)?'selected': '' ?>>True</option> 
                       <option value="false" <?= (set_value('isShown') == 0)?'selected': '' ?>>False</option> 
                  </select>
                </div> 
               </div>
              <hr>
              <div class="form-group row"> 
                ...
                <?php $varient_row = 0; ?>
                 <div class="col-sm-12">
                  <input type="hidden" name="is_varient" id="is_varient" value=""> 
  ...
  $(function () { 
    ...
    $("body").on("change","#categoryId",function(){ 
      $.post('<?=base_url($model."/users/getfranchiseproductsbycats")?>',
      {
        ...
        isglobal: '<?php echo $franchise['is_global'] ?>',
      },
      function(response){  
    ...
    var productId = '<?= set_value('productId'); ?>';
    ...
    $('.addvarient').on('click', function() {  
      var html ='<tr class="varientspro" id="varients-row' + varient_row + '" rid="' + varient_row + '" ><td><input type="text" class="form-control" name="varient[' + varient_row + '][measurment]" id="measurment' + varient_row + '"></td><td><select name="varient[' + varient_row + '][unit]" id="unit' + varient_row + '" class="form-control">';
        ...
        html +='<td><input type="text" name="varient[' + varient_row + '][price]" class="form-control" id="price' + varient_row + '" placeholder="" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g, \'\')"   value="0"></td> <td><input type="text" name="varient[' + varient_row + '][mrp]" class="form-control" id="mrp' + varient_row + '" placeholder="" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g, \'\')"   value="0"></td> <td><input type="text" name="varient[' + varient_row + '][disc_price]" class="form-control" id="disc_price' + varient_row + '" placeholder="" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g, \'\')"  value="0"></td>  <td><input type="text" name="varient[' + varient_row + '][qty]" class="form-control" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g, \'\')" id="qty' + varient_row + '"></td> <td><select name="varient[' + varient_row + '][is_active]" id="is_active' + varient_row + '" class="form-control"><option value="1">Available</option><option value="2">Not Available</option> <option value="3">Sold</option> </select></td>';
          if(is_wholesaler==1){
