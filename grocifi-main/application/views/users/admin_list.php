    ...
    <div class="card">
      <div class="card-header">
        <div class="d-inline-block">
          <h3 class="card-title"><i class="fa fa-list"></i>&nbsp; All Admins & Sub Admins List </h3>
        </div>
        <div class="d-inline-block float-right"> 
          <?php if($this->session->userdata('role_type')!='2' || (isset($this->general_user_premissions['user_admin_subadmin']['is_add']) && $this->general_user_premissions['user_admin_subadmin']['is_add']==1)){ ?> 
          <a href="<?= base_url($model.'/users/add/admin'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Admin/Subadmin </a>
        <?php  } ?>
        </div>
        <div  style="border-top: 1px solid #ddd;  width: 100%; margin-top: 15px; padding-top: 5px;" id="location-filter" class="col-sm-12 row" data-url="<?php echo base_url().$model.'/'; ?>" token-name="<?php echo $token_name; ?>" token-value="<?php echo $token_value; ?>"> 
                    <div class="col-sm-2">
                        <label for="search_role_id">Role</label>
                        <select name="search_role_id" class="form-control" id="search_role_id" > 
                        <option value="">Select Role</option> 
                        <?php foreach ($role as $key => $value) { ?>
                          <option value="<?=$value['role_code']?>"><?=$value['title']?></option>
                        <?php } ?>
                        </select>
                    </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url($model.'/users/admin_datatable_json')?>",
    "order": [[5,'desc']],
    "columnDefs": [
    ...
    { "targets": 6, "name": "is_active", 'searchable':true, 'orderable':false},
