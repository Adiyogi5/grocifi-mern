    ...
    <div class="card">
      <div class="card-header">
        <div class="d-inline-block">
          <h3 class="card-title"><i class="fa fa-list"></i>&nbsp; Offer List </h3>
        </div>
        <div class="d-inline-block float-right"> 
        <?php if($this->session->userdata('role_type')!='2' || (isset($this->general_user_premissions['offer']['is_add']) && $this->general_user_premissions['offer']['is_add']==1)){ ?>  
          <a href="<?= base_url($model.'/offer/add'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Offer</a>
        <?php } ?>  
        </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url($model.'/offer/offer_datatable_json')?>?franchise_id="+$('#search_franchise_id').val(),
    "order": [[1,'asc']],
    "columnDefs": [
  ...
  $("body").on("change",".tgl_checkbox",function(){ 
    $.post('<?=base_url($model."/offer/change_offer_status")?>',
    {
