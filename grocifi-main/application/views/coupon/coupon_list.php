    ...
    <div class="card">
      <div class="card-header">
        <div class="d-inline-block">
          <h3 class="card-title"><i class="fa fa-list"></i>&nbsp; Coupon List </h3>
        </div>
        ...
        <?php if($this->session->userdata('role_type')!='2' || (isset($this->general_user_premissions['coupon']['is_add']) && $this->general_user_premissions['coupon']['is_add']==1)){ ?>    
          <a href="<?= base_url($model.'/coupon/add'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Coupon</a>
        <?php } ?>  
        </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url($model.'/coupon/coupon_datatable_json')?>?franchise_id="+$('#search_franchise_id').val(),
    "order": [[1,'asc']],
    "columnDefs": [
    ...
    { "targets": 3, "name": "user", 'searchable':true, 'orderable':true},
    { "targets": 4, "name": "is_expire", 'searchable':false, 'orderable':false},
    { "targets": 5, "name": "dates", 'searchable':false, 'orderable':false},
    { "targets": 6, "name": "user_no", 'searchable':false, 'orderable':false},
    { "targets": 7, "name": "reuse", 'searchable':false, 'orderable':false},
    { "targets": 8, "name": "disc", 'searchable':false, 'orderable':false}, 
    { "targets": 9, "name": "is_active", 'searchable':false, 'orderable':true},
    { "targets": 10, "name": "Action", 'searchable':false, 'orderable':false,'width':'80px'}
    ]
  });
