      ...
      <div class="card card-default color-palette-bo">
        ...
        <div class="card-body">   
           <!-- For Messages -->
            ...
            <?php echo form_open(base_url($model.'/settings/edithoilday/'.$holiday['_id']), 'class="form-horizontal" id="holidayForm"');  ?> 
            <div class="form-group row">                
                  ...
                  <label for="holiday_date" class="col-sm-6 control-label">Franchise Name<span class="red">*</span></label>
                   <select name="franchiseId" class="form-control select2" id="franchiseId" >
                    ...
                    <?php foreach ($franchise as $key => $value) { ?>
                      <option  <?php if(set_value('franchiseId',$holiday['franchiseId'])==$value['_id']){ echo "selected='selected'"; }  ?> value="<?=$value['_id']?>"><?=$value['firmname']?></option>
                    <?php } ?>
                  </select>
              ...
              <div class="form-group row">                
                <div class="col-sm-6">
                  ...
                  <input type="date" name="holiday_date" class="form-control" id="holiday_date" placeholder="" value="<?= set_value('holiday_date',date('Y-m-d',strtotime($holiday['holiday_date']))); ?>">
                </div>
                  <div class="col-6">
                  <label for="is_active" class="col-sm-6 control-label">Select Status <span class="red">*</span></label>                  
                  <select name="is_active" class="form-control">
                    <option value="">Select Status</option>
                    <option value="1" <?= (set_value('is_active',$holiday['is_active']) == '1')?'selected': '' ?>>Active</option>
                    <option value="2" <?= (set_value('is_active',$holiday['is_active']) == '2')?'selected': '' ?>>Deactive</option>
                  </select>
                </div> 
              ...
              <div class="form-group row"> 
                ...
                <textarea name="description" class="form-control" id="description" ><?= set_value('description',$holiday['description']); ?></textarea> 
                </div>
              </div>
              <div class="form-group">
                <div class="col-md-12">
                  <input type="hidden" name="_id" value="<?=$holiday['_id']?>">
                  <input type="submit" name="submit" value="Update Holiday" class="btn btn-info pull-right">
                </div>
              </div>
            <?php echo form_close( ); ?>
          <!-- /.box-body -->
