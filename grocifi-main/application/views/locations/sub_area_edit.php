<?php $model = $this->session->userdata('model'); ?>
 <!-- Content Wrapper. Contains page content -->
      ...
      <div class="card card-default color-palette-bo">
        <div class="card-header">
          ...
          <div class="d-inline-block float-right">
            <a href="<?= base_url($model.'/locations/sub_area'); ?>" class="btn btn-success"><i class="fa fa-list"></i>Sub Area List</a>
          </div>
        </div>
        <div class="card-body">   
           <!-- For Messages -->
            <?php $this->load->view('includes/_messages.php') ?>
            <?php echo form_open(base_url($model.'/locations/editsub_area/'.$sub_area['_id']), 'class="form-horizontal" id="subareaForm"');  ?> 
              ...
              <div class="form-group row"> 
                <div class="col-6">
                  ...
                  <input type="text" name="title" class="form-control" id="title" placeholder="" value="<?= set_value('title',$sub_area['title']); ?>">
                </div> 
                <div class="col-sm-6">
                  <label for="is_active" class="col-sm-6 control-label">Select Status <span class="red">*</span></label>                  
                  <select name="is_active" class="form-control">
                    <option value="">Select Status</option>
                    <option value="1" <?= (set_value('is_active',$sub_area['is_active']) == '1')?'selected': '' ?>>Active</option>
                    <option value="2" <?= (set_value('is_active',$sub_area['is_active']) == '2')?'selected': '' ?>>Deactive</option>
                  </select>
                </div>
              </div>
              ...
              <div class="form-group">
                <div class="col-md-12">
                  <input type="hidden" name="_id" value="<?=$sub_area['_id']?>">
                  <input type="submit" name="submit" value="Update Sub Area" class="btn btn-info pull-right btn-submit">
                </div>
              </div>
            <?php echo form_close( ); ?>
          <!-- /.box-body -->
  ...
  $(document).ready(function(){   
    ...
    setTimeout(function(){  
      $("#cityId").val('<?= set_value('cityId',$sub_area['cityId']); ?>'); 
      $("#cityId").trigger('change'); 
    }, 1500);
    setTimeout(function(){  $("#areaId").val('<?= set_value('areaId',$sub_area['areaId']); ?>'); }, 2000);

     $("#subareaForm").validate({
      rules: {
      ...
      messages: {
          title: {
                  "required": "Please Enter Sub Area Name", 
                  "minlength": "Min Sub Area Name Should Be 3 Digits",
                  "maxlength": "Max Sub Area Name Should Be 50 Digits",
              },
