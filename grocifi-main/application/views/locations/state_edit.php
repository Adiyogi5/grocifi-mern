      ...
      <div class="card card-default color-palette-bo">
        ...
        <div class="card-body">   
           <!-- For Messages -->
            ...
            <?php echo form_open(base_url($model.'/locations/editstate/'.$state['_id']), 'class="form-horizontal" id="stateForm"');  ?> 
                ...
                <div class="col-sm-6">
                  ...
                  <select name="countryId" class="form-control" id="countryId" >
                    ...
                    <?php foreach ($country as $key => $value) { ?>
                      <option <?php if(set_value('countryId',$state['countryId'])==$value['_id']){ echo "selected='selected'"; }  ?> value="<?=$value['_id']?>"><?=$value['title']?></option>
                    <?php } ?>
                  </select>
                </div>
                  <div class="col-6">
                  <label for="title" class="col-sm-6 control-label">State Name <span class="red">*</span></label>
                  <input type="text" name="title" class="form-control" id="title" placeholder="" value="<?= set_value('title',$state['title']); ?>">
                </div>  
              </div>     
              <div class="form-group row">                
                <div class="col-sm-6">
                  ...
                  <select name="is_active" id="is_active" class="form-control">
                    ...
                    <option value="1" <?= (set_value('is_active',$state['is_active']) == '1')?'selected': '' ?>>Active</option>
                    <option value="2" <?= (set_value('is_active',$state['is_active']) == '2')?'selected': '' ?>>Deactive</option>
                  </select>
                </div>
              </div>
              ...
              <div class="form-group">
                <div class="col-md-12">
                  <input type="hidden" name="_id" id="_id" value="<?=$state['_id']?>">
                  <input type="submit" name="submit" value="Update State" class="btn btn-info pull-right btn-submit">
                </div>
              </div>
            <?php echo form_close( ); ?>
          <!-- /.box-body -->
