    ...
    <div class="card">
      <div class="card-header">
        <div class="d-inline-block">
          <h3 class="card-title"><i class="fa fa-list"></i>&nbsp; Group of Areas List </h3>
        </div>
        <div class="d-inline-block float-right"> 
          <?php if($this->session->userdata('role_type')!='2' || (isset($this->general_user_premissions['area_group']['is_add']) && $this->general_user_premissions['area_group']['is_add']==1)){ ?> 
          <a href="<?= base_url($model.'/locations/addarea_group'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add New Group of Areas</a>
          <?php } ?>
        </div>
      </div>
    </div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url($model.'/locations/area_group_datatable_json')?>",
    "order": [[1,'asc']],
    "columnDefs": [
    { "targets": 0, "name": "_id", 'searchable':false, 'orderable':false}, 
    { "targets": 1, "name": "title", 'searchable':true, 'orderable':true},
    { "targets": 2, "name": "cityName", 'searchable':true, 'orderable':true},
    { "targets": 3, "name": "stateName", 'searchable':true, 'orderable':true},
    { "targets": 4, "name": "created", 'searchable':false, 'orderable':true}, 
    { "targets": 5, "name": "is_active", 'searchable':false, 'orderable':true},
    { "targets": 6, "name": "Action", 'searchable':false, 'orderable':false,'width':'50px'}
    ]
  });
 
  $("body").on("change",".tgl_checkbox",function(){
    $.post('<?=base_url($model."/locations/change_area_group_status")?>',
    {
