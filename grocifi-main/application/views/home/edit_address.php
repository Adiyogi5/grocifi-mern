                            ...
                            <div class="row"> 
                                <div class="card">                                     
                                     <div class="user-card-header card-header">
                                         ...
                                         <div class="card-body">
                                             <?=form_open_multipart(base_url('edit_address/' . $address['_id']), 'id="editAddressForm", class="form-horizontal"')?>
                                            <div class="row">
                                                <div class="col-md-12 mb-md-3">
                                                    <label for="Address 1" class="form-label">Address 1 <span class="required">*</span></label>
                                                    <input type="text" class="form-control" name="address1" value="<?=set_value('address1', $address['address1'])?>" required> 
                                                </div>
                                                <div class="col-md-12 mb-md-3">
                                                    <label for="Address 1" class="form-label">Address 2 <span class="required">*</span></label>
                                                    <input type="text" class="form-control" name="address2" value="<?=set_value('address2', $address['address2'])?>" required>
                                                </div>
                                                <div class="col-md-6 mb-md-3">
                                                    <label for="Address 1" class="form-label">Address Type <span class="required">*</span></label>
                                                    <select name="address_type" id="address_type" class="form-control">
                                                        <?php foreach ($addressTypeArr as $addtypekey => $addtypevalue) {?>
                                                            <option value="<?=$addtypekey?>" <?=($addtypekey == $address['address_type']) ? 'selected' : ''?>><?=$addtypevalue?></option>
                                                        <?php }?>
                                                    </select>
                                                </div>
                                                <div class="col-md-6 mb-md-3">
                                                    ...
                                                    <select id="search_country" name="countryId" class="form-control custom-select border-form-control">
                                                      <option value="0">Select Country</option>
                                                       <?php foreach ($search_country as $val) { ?>
                                                           <option value="<?php echo $val["_id"]; ?>" <?=($val["_id"] == $address['countryId']) ? 'selected' : ''?>><?php echo $val["title"]; ?></option>
                                                       <?php } ?>
                                                   </select>
                                                </div>
                                                <div class="col-md-6 mb-md-3">
                                                ...
                                                <div class="col-md-6 mb-md-3">
                                                    <label for="pincode" class="form-label">Zip Code <span class="required">*</span></label>
                                                    <input type="number" class="form-control" name="pincode" placeholder="eq-123456" value="<?=$address['pincode']?>" required>
                                                </div>
                                                <div class="col-md-6 mb-md-3">
                                                    <input type="checkbox" class="custom-control-input" id="default_address" name="default_address" value="true" <?=($address["default_address"]) ? "checked" : "";?>>
                                            ...
                                            <div class="row">
                                                 <div class="form-group">
                                                  ...
                                                  <div class="col-sm-12"> 
                                                  <input type="hidden" name="long" id="long" value="<?=set_value('long', $address['long'])?>">
                                                  <input type="hidden" name="lat" id="lat" value="<?=set_value('lat', $address['lat'])?>">
                                            ...
                                            <div class="row">
                                                <div class="col-sm-12 d-flex justify-content-end">
                                                    ...
                                                    <input type="hidden" name="from_cart" value="<?php echo (isset($_GET["fromcart"]) ? "true" : "false") ?>"> 
                                                     <button id="update_user_address" type="submit" class="btn btn-success btn-lg"> Save Address </button>
                                                </div>
                                            </div>
                                            </form>
                                        </div>
...
<script type="text/javascript">
     $(document).ready(function(){
       ...
       setTimeout(function () { 
            ...
            setTimeout(function () { $("#search_state").val('<?=set_value('stateId', $address['stateId'])?>'); }, 700);

            /// get state
            setTimeout(function () {                 
                $("#search_state").trigger("change"); 
                setTimeout(function () { $("#search_city").val('<?=set_value('cityId', $address['cityId'])?>'); }, 700); 

                // get city
                setTimeout(function () { 
                    $("#search_city").trigger("change"); 
                    setTimeout(function () { $("#search_area").val('<?=set_value('areaId', $address['areaId'])?>'); }, 700);                
                }, 1000);
            }, 800);
        }, 500);
        ...
        $('body').on('click','.update_user_address',function(){
