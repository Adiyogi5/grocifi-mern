...
class Lucky_draw_coupon extends MY_Controller {
	...
	public function offer_datatable_json(){
		...
		if(@$offer_data['sucess']=='200'){  		
			...
			{  
				...
				$edt = '';
				if($row['is_lock'] == '0'){
					$edt = '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url($this->model.'/lucky_draw_coupon/edit/'.$row['_id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
				}
				if($row['is_lock'] == '1' && $row['is_generate'] == '0'){
					$edt = '<a title="Edit Offer Date" class="update btn btn-sm btn-warning" href="'.base_url($this->model.'/lucky_draw_coupon/editofferdate/'.$row['_id']).'"> <i class="fa fa-calendar"></i></a>';
				}
				...
				if($is_lock=='0'){
					$lockData = '<input class="tgl_checkbox tgl-ios" 
					data-id="'.$row['_id'].'" 
					id="cb_'.$row['_id'].'"
					type="checkbox"  
					'.$status.'><label for="cb_'.$row['_id'].'"></label>';
				}elseif($is_lock=='1' && $row['is_generate']==1){
					$lockData = 'Winner Declared';
				}elseif($is_lock=='1'){
					$lockData = 'Locked';
				} 
	...
	public function edit($id = 0){ 
		if($this->input->post('submit')){
			...
			if($data['offer']['is_lock']){
				$this->session->set_flashdata('error', 'You cannot edit "Lucky Draw Coupon"!!');
				redirect(base_url($this->model.'/lucky_draw_coupon'));
			}
			$authtoken = $this->session->userdata('authtoken'); 
			$url =  $this->config->item('APIURL') . 'franchise/index';		
	...
	//-----------------------------------------------------------
	public function editofferdate($id = 0){ 
		if($this->input->post('submit')){ 
			$this->form_validation->set_rules('expiry_date', 'End Date', 'trim|required|strip_tags|xss_clean'); 
			$this->form_validation->set_rules('description', 'Description', 'trim|required|strip_tags|xss_clean'); 
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				$this->load->view('includes/_header');
				$this->load->view('lucky_draw_coupon/offer_editdate', $data);
				$this->load->view('includes/_footer');
			}else{
				$sdate = $this->input->post('start_date');
				$edate = $this->input->post('expiry_date');
				...
				$data = array( 
					...
					'description'=> $this->input->post('description'),
					'modifiedby' =>  $this->session->userdata('admin_id'), 
					'offerimg' => $this->input->post('offerimg') 
				);  
				/*if(strtotime($sdate) < strtotime($today)){
					$dateError = 'You can not create lucky draw coupon in back date'; 
				}*/
				...
				if($dateError!=''){	  
					...
					$this->session->set_flashdata('errors', $edata['errors']);
					$this->load->view('includes/_header');
					$this->load->view('lucky_draw_coupon/offer_editdate', $edata); 
					$this->load->view('includes/_footer');
				}else{  
					$data = array( 
						'_id' => $this->input->post('_id'),
 	...
 	public function updateproduct($id = 0){
		...
		$url =  $this->config->item('APIURL') . 'lucky_draw_coupon/getparticateusers/'.$id;
		$records = $this->Commonmodel->getData($url, $authtoken); 
		if($records['sucess']=='200'){
			$data['particateuserslist'] = $records['data'];
		} 
		///  echo "<pre>";print_r($data['particateuserslist']); exit;
		$this->load->view('includes/_header', $data);
	...
	public function generatewinner(){   
		$authtoken = $this->session->userdata('authtoken');  
		$url =  $this->config->item('APIURL') . 'lucky_draw_coupon/generatewinner';
		$data =  $this->input->post(); 
		$records = $this->Commonmodel->postData($url, $data, $authtoken); 
		echo json_encode($records);
	}
