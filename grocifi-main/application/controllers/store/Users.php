...
class Users extends MY_Controller {
	...
	public function franchise_datatable_json(){				   					   
		...
		$url =  $this->config->item('APIURL') . 'franchise/edit/'.$franchise_id; 
		$user_data = $this->Commonmodel->getData($url, $authtoken); 
		$userData = $user_data['data'];
		$data = array(); 

		if(@$user_data['sucess']=='200'){  		 			
			$records = array("recordsTotal"=>1,"recordsFiltered"=>1,'data' => $data);	
			$edit = '';
			if($this->session->userdata('role_type')!='2' || $this->general_user_premissions['user_franchise']['is_edit']==1){
					$edit = '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url($this->model.'/users/edit/'.$userData['userId'].'/franchise').'"> <i class="fa fa-pencil-square-o"></i></a>';
				}
			$data[]= array(
				'1', 
				'<a title="Franchise Detail" href="'.base_url($this->model.'/users/franchise_detail/'.$userData['userId'].'').'">'.$userData['firmname'].'</a>',  
				isset($userData['ownermobile'])?$userData['ownermobile']:' - ', 
				date_time($userData['created']),	
				'Active',	
				$edit
			);
	...
	public function add($mode=NULL){
		...
		$modelist= array('admin','franchise','customer','wholesaler','delivery_boy');  
		...
		if($mode=='delivery_boy' || $mode=='admin'){
			...
			$franchise[0]['_id']=$franchise_id;
			$franchise[0]['firmname']= $firmname; 
			$data['franchise_list'] = $franchise; 
		}
		$data['franchise_id'] = $this->session->userdata('franchise_id');
	...
	function franchise_detail($id = null){
		...
		if($data['franchise'][0]['franchise'][0]['_id']==$franchise_id){
			$data['id'] = $id;
	...
	public function getfranchiseproductsbycats(){ 
		...
		$authtoken = $this->session->userdata('authtoken'); 
		if(!empty($categoryid)){
			$url =  $this->config->item('APIURL') . 'product/getproductsbycats/'.$categoryid;
			$records = $this->Commonmodel->getData($url, $authtoken);	
			$product = '';   
	...
	public function manage_permission($id){  
		...
		$perrecord = $this->Commonmodel->getData($url, $authtoken); 
  		...
  		/// Get all Modules
  		$notval = array('category','product','country','state','city','area','area_group','sub_area','general_notification','settings','cms','admanager','delivery_time_slot','rolemanager','coupon_todayexpiry');
		...
		$modulerecord = $this->Commonmodel->getData($url, $authtoken); 
  		if($modulerecord['success']=='200'){ 
  			foreach ($modulerecord['data'] as $key => $value) {
  				if(!in_array($value['name'], $notval)){
	  				$allmodule[] = $value;
	  			}
  			} 
  		}  
