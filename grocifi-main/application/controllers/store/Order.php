...
class Order extends MY_Controller {
	...
	public function order_index_json(){
		...
		if(@$order_data['sucess']=='200'){  		
			...
			foreach ($order_data['data']  as $row){
				...
				if ($this->session->userdata('role_type')!=3){
					$userDetail = '<span style="color:'.$color.';">'.$row["full_name"].'<br>('.$row["phone_no"].')</span>';
				}else{
					$userDetail = '<span style="color:'.$color.';">'.$row["full_name"].'';
				}
	...
	public function purchase_list(){
		...
		$url =  $this->config->item('APIURL') . "order/getcitodayorder/$start_date/$end_date/$franchise_id";
		$temp = $this->Commonmodel->getData($url, $authtoken);
	...
	public function purchased_list(){
		...
		$url =  $this->config->item('APIURL') . "franchise/getcipurchaseditembydate/$today/$franchise_id";
		$temp = $this->Commonmodel->getData($url, $authtoken);
		$data['list']= $temp["data"];
	...
	public function report(){
		...
		$url =  $this->config->item('APIURL') . "order/getciorderbydate/$start_date/$end_date/$franchise_id";
		$temp = $this->Commonmodel->getData($url, $authtoken);
		//prd($temp);
		$temp = $temp["data"];
		$temp_1 = array();
	...
	public function placeorder(){
		$franchise_id = $this->session->userdata('franchise_id');
		$franchiseSetting = auth_user('franchise');
		if($this->input->post('submit')){			 
			if($_SESSION['CartTotal']['user_wallet']!=0){ $key_wallet_used = 1; }else{ $key_wallet_used = 0; }
			$authtoken = $this->session->userdata('authtoken');  
			$url =  $this->config->item('APIURL') . "settings/getconfigData?franchiseId=".$franchise_id;
			$_configs = $this->Commonmodel->getData($url, $authtoken);
			$_configs = $_configs["data"];
			$temp = array();
			foreach($_configs[2] as $val){
			...
			$franchise[0]['firmname']= $firmname; 
			$data['franchise'] = $franchise;
			$data['delivery_max_day'] = $franchiseSetting['delivery_max_day'];
			$data['delivery_day_after_order'] = $franchiseSetting['delivery_day_after_order'];
	...
	function checkorderdeliverydata(){
		...
		$min_delivery_date = date('Y-m-d', strtotime($delivery_date. ' + '.$this->general_settings['delivery_day_after_order'].' days')); 
		$delivery_max_day = date('Y-m-d', strtotime($todaydate. ' + '.$this->general_settings['delivery_max_day'].' days')); 
		if(strtotime($delivery_date) >= strtotime($delivery_max_day)){
			$records['status'] = '2';
			$records['msg'] = $this->general_settings['site_name']." can not allow delivery after ".$this->general_settings['delivery_max_day']." days!!";
		}else{
			$records['status'] = '1';
	...
	function addcartinsession(){
		...
		if($_SESSION['is_wholesaler']=='true'){
			$minOrderVal = $this->general_settings['min_order_wholesaler'];
		}else{
			$minOrderVal = $this->general_settings['min_order'];
		}	
		if(($this->general_settings['accept_minimum_order']==false) 
			&&  ($total) < (float)$minOrderVal){
			$delivery_charge = $this->general_settings['delivery_chrge'];
		}else{
